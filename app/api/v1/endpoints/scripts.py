...
async def generate_script(
    ...
    try:
        logger.info(
            f"Script generation request: topic='{request.topic}', "
            f"type={request.video_type}, input_type={request.input_type}"
        )

        # Validate input
        if request.input_type in ["VIDEO", "BLOG"] and not request.video_link:
            logger.warning(f"Missing video_link for input_type {request.input_type}")
            raise BadRequestException("Link is required for video and blog input types")
        ...
        logger.info(f"Script generation initiated with task_id: {task_id}")
...
async def regenerate_script(
    ...
    try:
        ...
        script_data = await script_service.get_script_of_user(user_id, script_id)
        if not script_data:
