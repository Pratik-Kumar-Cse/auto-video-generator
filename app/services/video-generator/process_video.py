...
import os
from app.helpers.progress_logger import ProgressLogger
from app.templates.video.reel_template import SHORT_TEMPLATES
from app.templates.video.video_template import VIDEO_TEMPLATES


class VideoTemplateProcessor:
    def __init__(self, is_video: bool = False, template_id="template_1"):
        """
        ...
        """
        self.template = (
            VIDEO_TEMPLATES[template_id] if is_video else SHORT_TEMPLATES[template_id]
        )
        self.width = self.template["resolution"]["width"]
        self.height = self.template["resolution"]["height"]
        self.aspect_ratio = self.template["aspect_ratio"]
        self.fps = self.template["fps"]
        self.duration = None
        self.blur_amount = 60
        self.main_content_clip = None
    ...
    def process_element(self, element, clip_data_object):
        ...
        if element["type"] == "video":  # Process video elements
            ...
            elif element["id"] == "main_content":  # Handle main content element
                self.main_content_clip = clip  # Store main content clip
    ...
    def generate_video(self, source_files, output_path, video_id):
        """
        ...
        try:
            ...
            if source_files and source_files.get("intro_clip") is not None:
                intro_clip = source_files["intro_clip"].resize(
                    (self.width, self.height)
                )
            ...
            if "main_content" in source_files:
                intro_time = current_time
                # Process main content
                main_element = next(
                    (e for e in self.template["elements"] if e["id"] == "main_content"),
                    None,
                )
                if main_element:
                    main_clip = self.process_element(
                        main_element, {"video_clip": source_files["main_content"]}
                    )
                    ...
                    if "background" in source_files:
                        ...
                        background_clip = self.create_blurred_background(
                            self.main_content_clip
                        )
                        background_clip = background_clip.set_start(current_time)
                        main_section_clips.append(background_clip)
                    ...
                    if "stock" in source_files:
                        ...
                        end_time = 0
                        video_path = f"./generated_videos/{video_id}/videos"
                        stock_video_path = f"{video_path}/stock"
                        image_video_path = f"{video_path}/images"
                        stock_video_placement = source_files["stock"]
                        image_index = 0
                        video_index = 0
                        for index, stock_video in enumerate(stock_video_placement):

                            relevant_segments = stock_video.get("relevantSegments", [])
                            video_start_time = 0
                            if len(relevant_segments) > 0:
                                video_start_time = relevant_segments[0]["start_time"]
                            ...
                            if "image" in stock_video:
                                image_path = f"{image_video_path}/{image_index}.jpg"
                                if os.path.exists(video_path):
                                    stock_clip = self.process_element(
                                        clip_element,
                                        {
                                            "video_clip": ImageClip(
                                                image_path, duration=20
                                            )
                                        },
                                    )
                            ...
                            if "search_terms" in stock_video:
                                video_path = f"{stock_video_path}/{video_index}.mp4"
                    ...
                    if "logo" in source_files:
                        ...
                        if logo_element:
                            logo_clip = self.process_element(
                                logo_element, {"source_path": source_files["logo"]}
                            )
            ...
            if source_files and source_files.get("outro_clip") is not None:
                outro_clip = source_files["outro_clip"].resize(
                    (self.width, self.height)
                )
            ...
            final.write_videofile(
                ...
                audio_codec="aac",
                fps=fps,
                logger=ProgressLogger(video_id, fps, final_duration),
                remove_temp=True,
            )
            final.close()
            ...
            for clip in all_clips:
                try:
                    ...
                    pass
            if self.main_content_clip:
                try:
                    self.main_content_clip.close()
                except:
                    pass
    ...
    def close(self):
        ...
        if hasattr(self, "main_content_clip"):
            self.main_content_clip.close()
