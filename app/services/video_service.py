...
class VideoService:
    ...
    async def upload_video(
        ...
        try:
            ...
            await self.update_video(video["_id"], {
                "upload_info": {
                    upload_domain: upload_result
                }
            })
    ...
    async def generate_video_using_workflow(
        ...
        try:
            ...
            raise VideoProcessingException(f"Failed to generate workflow video: {str(e)}")
    ...
    async def generate_video_using_workflow_with_video_id(
        ...
        try:
            ...
            raise VideoProcessingException(f"Failed to generate workflow video by ID: {str(e)}")
    ...
    async def process_script_for_video(
        ...
        try:
            ...
            if (video_data.get("stock_video_clips") is None or 
                len(video_data.get("stock_video_clips", [])) == 0):
                ...
                video_data = await self.update_video(video_id, {
                    "stock_video_clips": stock_clips,
                    "video_urls": video_urls
                })
            
            # Generate images if needed
            if (video_data.get("images") is None or 
                len(video_data.get("images", [])) == 0):
    ...
    async def fetch_stock_videos(
        self,
        video_id: str,
        search_terms_data: Dict[str, Any]
    ...
    async def generate_audio_data(
        ...
        try:
            ...
            return {
                "_id": str(ObjectId()),
                "audio_links": [
                    f"https://example.com/audio/{uuid.uuid4()}.mp3"
                ],
                "subtitles": [
                    {
